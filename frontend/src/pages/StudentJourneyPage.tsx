...
function StudentHome({ preview = false }: { preview?: boolean }) {
  ...
  return <div className="space-y-6"><EditorialPageHeader eyebrow="Student journey · personal services" title={preview ? '学生门户预览' : `你好，${user?.real_name || user?.username || '同学'}`} description={preview ? '当前仅切换前端视图，所有请求仍使用管理员本人权限，不会模拟学生身份。' : '从咨询服务进入课程与活动，也可以查看当前学生服务的开放范围。'} />
    <ArchiveCard title="咨询与活动" index="AVAILABLE"><div className="grid gap-4 md:grid-cols-2"><Link to="/customer-service" className="group border border-wine/30 bg-wine/5 p-5 transition-colors hover:border-wine hover:bg-wine/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><MessageSquare className="h-6 w-6 text-wine" aria-hidden /><h2 className="mt-4 font-serif text-xl font-semibold">进入客服咨询</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">保存自己的咨询会话，查找课程和近期活动，并完成报名或取消报名。</p><span className="mt-5 flex items-center gap-2 text-xs font-semibold text-wine">打开咨询中心 <ArrowRight className="h-3 w-3" aria-hidden /></span></Link><div className="border border-bronze/35 p-5"><GraduationCap className="h-6 w-6 text-wine" aria-hidden /><h2 className="mt-4 font-serif text-xl font-semibold">学生服务说明</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">系统只展示后端已开放的真实能力；成绩、签证与申请进度不会使用模拟数据。</p><StatusStamp label="真实数据边界" tone="info" /></div></div></ArchiveCard>
    <ArchiveCard title="后续服务" index="PLANNED"><div className="grid gap-4 md:grid-cols-3">{unavailable.map((item) => { const Icon = item.icon; return <article key={item.title} className="border-l border-bronze pl-4"><Icon className="h-5 w-5 text-wine" aria-hidden /><h3 className="mt-3 text-sm font-medium">{item.title}</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">{item.description}</p><div className="mt-3"><StatusStamp label="接口未开放" tone="neutral" /></div></article> })}</div></ArchiveCard>
  </div>
}
